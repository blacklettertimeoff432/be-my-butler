...
| 기존 워크플로우 | BMB |
|---|---|
| 한 에이전트가 모든 걸 처리 | 10개 전문 에이전트가 역할 분담 |
...
- **12단계 풀 파이프라인** — Session Prep → Consulting → Council Debate → Architecture → Execution → Testing → Blind Verification → Simplification → **Analyst (Step 10.5)** → Documentation → Learning → Handoff
...
12단계 파이프라인의 전체 흐름을 시각적으로 확인하세요:
